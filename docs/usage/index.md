...
这里回答三个实际问题：怎样把 Go 后台管理基础服务跑起来、怎样选择部署入口、怎样判断它能否接你的流量。`system` 已覆盖登录权限、租户和通知等能力，`infra` 已覆盖文件、配置、代码生成与日志等能力；固定 Vben 管理端随 Git 子模块提供。当前 `0.0.1` 适合本机开发和隔离环境联调，接入线上前必须完成[兼容性验收](compatibility.md)。
